...
# Opt-in: only initialises when SENTRY_DSN is set, so no account is required
# to run the app. See SECURITY.md for what gets sent.
...
def _check_session_rate_limit(session_id: str) -> None:
    ...
    with _session_hits_lock:
        ...
        if len(_session_hits) > 500:
            stale = [
                sid
                for sid, ts_list in _session_hits.items()
...
def escalate_chat(request: Request, body: EscalateRequest):
    try:
        ...
        raise HTTPException(status_code=500, detail="Could not escalate. Please try again.") from e
...
def feedback(request: Request, body: FeedbackRequest):
    try:
        ...
        raise HTTPException(status_code=500, detail="Could not save feedback. Please try again.") from e
...
def history(session_id: str):
    try:
        ...
        raise HTTPException(status_code=500, detail="Could not retrieve history. Please try again.") from e
...
def ticket(request: Request, body: TicketRequest):
    try:
        ...
        raise HTTPException(status_code=500, detail="Could not submit ticket. Please try again.") from e
